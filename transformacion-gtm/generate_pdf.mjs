...
const htmlPath = path.join(__dirname, 'deck_transformacion.html');
const pdfPath = path.join(__dirname, 'Estrategia_Transformacion_GTM_Deck.pdf');
...
for (let i = 0; i < slideCount; i++) {
  ...
  const screenshotPath = path.join(__dirname, `_slide_${i}.png`);
  await page.screenshot({ path: screenshotPath, clip });
  screenshots.push(screenshotPath);
  console.log(`Captured slide ${i + 1}/${slideCount}`);
}

// Create a new HTML page with one image per page for PDF
